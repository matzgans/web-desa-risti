    ...
    <div class="pt-4">
        <div class="max-w-7xl px-0 sm:px-6 lg:px-8">
            <div class="mb-3 flex justify-between rounded-lg bg-secondary p-4 text-white shadow-lg">
                ...
                <h2>Edit Aparat Desa</h2>
                <a class="flex items-center hover:underline" href="{{ route('admin.structure.index') }}">
            ...
            <div class="rounded-lg bg-white p-4 text-white shadow-lg">
                ...
                <form class="mx-auto max-w-full"
                    action="{{ route('admin.structure.update', ['structure' => $structure->uuid]) }}" method="POST"
                    ...
                    <div class="grid w-full grid-cols-2 gap-4">
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                for="staff_name">Nama Pegawai</label>
                            <input
                                ...
                                id="staff_name" name="staff_name" type="text"
                                value="{{ old('staff_name', $structure->staff_name) }}" placeholder="John Anderson...."
                                required />
                            @error('staff_name')
                        ...
                        <div>
                            ...
                            <select
                                ...
                                @foreach ($categories as $item)
                                    <option value="{{ $item }}"
                                        {{ old('position', $structure->position) == $item ? 'selected' : '' }}>
                                        {{ $item }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                for="staff_description">Deskripsi</label>
                            <input
                                class="block w-full rounded-lg border border-secondary bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary focus:ring-secondary dark:border-secondary dark:bg-gray-700 dark:text-white"
                                id="staff_description" name="staff_description" type="text"
                                value="{{ old('staff_description', $structure->staff_description) }}"
                                placeholder="Periode 2024-2029" required />
                            @error('staff_name')
                                <span class="text-sm text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input
                                class="block w-full rounded-lg border border-secondary bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary focus:ring-secondary dark:border-secondary dark:bg-gray-700 dark:text-white"
                                id="nip" name="nip" type="text"  value="{{ old('nip', $structure->nip) }}" placeholder="1234567890" required />
                            @error('nip')
                                <span class="text-sm text-red-500">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="col-span-2 mb-6 grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <label
                                    ...
                                    <input class="hidden" id="dropzone-file" name="staff_photo" type="file"
