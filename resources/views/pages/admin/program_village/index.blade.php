<x-app-layout>
    <div class="pt-4">
        <div class="max-w-full px-3 sm:px-6 lg:px-8">
            ...
            <div class="rounded-lg bg-white p-4 text-white shadow-lg">
                ...
                <div class="mb-2 flex flex-col items-center sm:flex-row">
                    <form class="mb-2 w-full sm:mb-0 sm:me-2" action="{{ route('admin.villageprogram.index') }}"
                        method="GET">
                    ...
                    {{-- <a class="mb-2 w-full rounded-lg bg-red-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-300 focus:outline-none focus:ring-4 focus:ring-red-300 dark:bg-red-300 dark:hover:bg-red-500 dark:focus:ring-red-300 sm:mb-0 sm:me-2 sm:w-auto"
                        href="{{ route('admin.export.village') }}">
                ...
                <div class="relative overflow-x-auto">
                    <table
                        ...
                        <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th class="border border-gray-300 px-6 py-3" scope="col">Nama Program</th>
                                <th class="border border-gray-300 px-6 py-3" scope="col">Isi Program</th>
                                <th class="border border-gray-300 px-6 py-3" scope="col">Kategori Program</th>
                                <th class="border border-gray-300 px-6 py-3" scope="col">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($program_villages as $program_village)
                                <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                                    <th class="whitespace-nowrap border border-gray-300 px-6 py-4 font-medium text-gray-900 dark:text-white"
                                        scope="row">
                                        {{ ($program_villages->currentPage() - 1) * $program_villages->perPage() + $loop->iteration }}
                                    </th>
                                    <td class="border border-gray-300 px-6 py-4">
                                        {{ $program_village->program_name }}

                                    </td>
                                    <td class="border border-gray-300 px-6 py-4">
                                        {{ $program_village->description }}
                                    </td>
                                    <td class="border border-gray-300 px-6 py-4">
                                        {{ $program_village->program_category }}
                                    </td>
                                    <td class="border border-gray-300 px-6 py-4">
                                        <div class="flex items-center">

                                            <!-- Tombol Edit Kategori -->
                                            <a class="edit-village_name" data-modal-target="crud-modal-edit"
                                                data-modal-toggle="crud-modal-edit"
                                                data-slug="{{ $program_village->slug }}"
                                                data-name="{{ $program_village->program_name }}"
                                                data-desc="{{ $program_village->description }}"
                                                data-program_category="{{ $program_village->program_category }}">
                                            ...
                                            <a href="javascript:void(0);"
                                                onclick="confirmDelete('{{ $program_village->slug }}', '{{ addslashes($program_village->program_name) }}')">
                                            ...
                                            <form id="delete-form-{{ $program_village->slug }}"
                                                style="display: none;"
                                                action="{{ route('admin.villageprogram.destroy', ['villageprogram' => $program_village->slug]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                    ...
                    {{ $program_villages->links('vendor.pagination.tailwind') }}
                </div>
            </div>
        </div>
        ...
        <div class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
            id="crud-modal" aria-hidden="true" tabindex="-1">
            <div class="relative max-h-full w-full max-w-md p-4">
                ...
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" method="POST" action="{{ route('admin.villageprogram.store') }}">
                        @csrf
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                    for="program_name">Nama Program</label>
                                <input
                                    class="focus:ring-primary-600 focus:border-primary-600 dark:focus:ring-primary-500 dark:focus:border-primary-500 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
                                    id="program_name" name="program_name" type="text" placeholder="Nama Progran"
                                    required
                                    oninput="this.value = this.value.replace(/\b\w/g, function(char) { return char.toUpperCase(); })">
                                @error('program_name')
                            ...
                            <div class="col-span-2">
                                <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                    for="description">Isi Program</label>
                                <textarea
                                    ...
                                    id="description" name="description" placeholder="John Anderson...." required rows="4">1. </textarea>
                                @error('description')
                                    <span class="text-sm text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-span-2">
                                <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                    for="program_category">Kategori Program</label>
                                <select
                                    class="block w-full rounded-lg border border-secondary bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary focus:ring-secondary dark:border-secondary dark:bg-gray-700 dark:text-white"
                                    id="program_category" name="program_category" required>
        ...
        <!-- Main modal update -->
        <div class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
            id="crud-modal-edit" aria-hidden="true" tabindex="-1">
            <div class="relative max-h-full w-full max-w-md p-4">
                <!-- Modal content -->
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    <!-- Modal header -->
                    <div class="flex items-center justify-between rounded-t border-b p-4 dark:border-gray-600 md:p-5">
                        ...
                        <button
                            class="ms-auto inline-flex h-8 w-8 items-center justify-center rounded-lg bg-transparent text-sm text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white"
                            data-modal-toggle="crud-modal-edit" type="button">
                            <svg class="h-3 w-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                fill="none" viewBox="0 0 14 14">
                                <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                    stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" id="edit-form" method="POST"
                        action="{{ route('admin.villageprogram.update', ['villageprogram' => ':slug']) }}">
                        @method('PUT')
                        @csrf
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            ...
                            <div class="col-span-2">
                                ...
                                <input
                                    class="focus:ring-primary-600 focus:border-primary-600 dark:focus:ring-primary-500 dark:focus:border-primary-500 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
                                    id="program_name_update" name="program_name" type="text"
                                    placeholder="Nama Progran" required
                                    oninput="this.value = this.value.replace(/\b\w/g, function(char) { return char.toUpperCase(); })">
                                @error('program_name')
                                    <span class="text-sm text-red-500">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea
                                    class="block w-full rounded-lg border border-secondary bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary focus:ring-secondary dark:border-secondary dark:bg-gray-700 dark:text-white"
                                    id="description_update" name="description" placeholder="John Anderson...." required rows="4">1. </textarea>
                                @error('description')
                                    <span class="text-sm text-red-500">{{ $message }}</span>
                                @enderror
                            </div>
                            <div class="col-span-2">
                                ...
                                <select
                                    class="block w-full rounded-lg border border-secondary bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-primary focus:ring-secondary dark:border-secondary dark:bg-gray-700 dark:text-white"
                                    id="program_category_update" name="program_category" required>
                                    <option value="">Pilih Kategori Program</option>
                                    <option value="Tambahan">
                                        Tambahan
                                    </option>
        ...
        <script>
            document.getElementById('description').addEventListener('keydown', function(event) {
                if (event.key === 'Enter') {
                    ...
                    const lines = currentValue.split('\n'); // Pisahkan baris yang ada
                    const nextNumber = lines.length + 1; // Hitung nomor berikutnya
                    const newValue = currentValue + '\n' + nextNumber + '. '; // Tambahkan nomor baru
                    this.value = newValue;
                }
            });
            document.addEventListener('DOMContentLoaded', function() {
                const editButtons = document.querySelectorAll('.edit-village_name');
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        const program_category = this.getAttribute('data-program_category');
                        console.log(name, desc, program_category);


                        // Ambil form dan ganti action dengan ID
                        const form = document.querySelector('#edit-form');
                        form.action = form.action.replace(':slug',
                            slug); // Ganti placeholder dengan ID yang sesuai

                        // Set nilai input nama kategori
                        const inputprogam_name = document.querySelector('#program_name_update');
                        inputprogam_name.value = name;
                        const inputprogam_desc = document.querySelector('#description_update');
                        inputprogam_desc.value = desc;
                        const inputProgramCategory = document.querySelector('#program_category_update');

                        // Cari opsi yang memiliki nilai sesuai dan tandai sebagai 'selected'
                        const options = inputProgramCategory.options;
                        for (let i = 1; i < options.length; i++) {
                            if (options[i].value === program_category) {
                        ...
                        // Tampilkan modal
                        const modal = document.getElementById('crud-modal-edit');
