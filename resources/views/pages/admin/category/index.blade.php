        ...
        <div class="max-w-full px-3 sm:px-6 lg:px-8">
            ...
            <div class="rounded-lg bg-white p-4 text-white shadow-lg">
                <div class="mb-3 flex w-full max-w-full items-center rounded-lg bg-white p-4 text-gray-500 shadow dark:bg-gray-800 dark:text-gray-400"
                    ...
                    <div class="ms-3 text-sm font-normal">Data Kategor ini nantinya akan di pakai saat menginputkan
                        struktur aparat desa</div>
                    <button
                ...
                <div class="mb-2 flex flex-col items-center sm:flex-row">
                    <form class="mb-2 w-full sm:mb-0 sm:me-2" action="{{ route('admin.staffcategory.index') }}"
                        method="GET">
                    ...
                    {{-- <a class="mb-2 w-full rounded-lg bg-red-500 px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-red-300 focus:outline-none focus:ring-4 focus:ring-red-300 dark:bg-red-300 dark:hover:bg-red-500 dark:focus:ring-red-300 sm:mb-0 sm:me-2 sm:w-auto"
                        href="{{ route('admin.export.staffcategory') }}">
                ...
                <div class="relative overflow-x-auto">
                    <table
                        ...
                        <thead class="bg-gray-50 text-xs uppercase text-gray-700 dark:bg-gray-700 dark:text-gray-400">
                            <tr>
                                ...
                                <th class="border border-gray-300 px-6 py-3" scope="col">Kategori</th>
                                <th class="border border-gray-300 px-6 py-3" scope="col">Aksi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($staff_categories as $staffcategory)
                                <tr class="border-b bg-white dark:border-gray-700 dark:bg-gray-800">
                                    <th class="whitespace-nowrap border border-gray-300 px-6 py-4 font-medium text-gray-900 dark:text-white"
                                        scope="row">
                                        {{ ($staff_categories->currentPage() - 1) * $staff_categories->perPage() + $loop->iteration }}
                                    </th>
                                        ...
                                        <div class="flex items-center">
                                            @if ($staffcategory->category !== 'Kepala Desa Lama' && $staffcategory->category !== 'Kepala Desa')
                                                ...
                                                <a class="edit-category" data-modal-target="crud-modal-edit"
                                                    ...
                                                    data-id="{{ $staffcategory->id }}"
                                                    data-name="{{ $staffcategory->category }}">
                                                    <svg class="size-6 text-yellow-500 dark:text-white"
                                                ...
                                                <a href="javascript:void(0);"
                                                    onclick="confirmDelete('{{ $staffcategory->id }}', '{{ addslashes($staffcategory->category) }}')">
                                            ...
                                            @endif
                                            <form id="delete-form-{{ $staffcategory->id }}" style="display: none;"
                                                action="{{ route('admin.staffcategory.destroy', ['staffcategory' => $staffcategory->id]) }}"
                                                method="POST">
                                                @csrf
                                                @method('DELETE')
                                            </form>
                    ...
                    {{ $staff_categories->links('vendor.pagination.tailwind') }}
                </div>
            </div>
        </div>
        ...
        <div class="fixed left-0 right-0 top-0 z-50 hidden h-[calc(100%-1rem)] max-h-full w-full items-center justify-center overflow-y-auto overflow-x-hidden md:inset-0"
            ...
            <div class="relative max-h-full w-full max-w-md p-4">
                ...
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    ...
                    <!-- Modal body -->
                    <form class="p-4 md:p-5" method="POST" action="{{ route('admin.staffcategory.store') }}">
                        @csrf
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                <label class="mb-2 block text-sm font-medium text-gray-900 dark:text-white"
                                    for="category">Nama Categori</label>
                                <input
                                    class="focus:ring-primary-600 focus:border-primary-600 dark:focus:ring-primary-500 dark:focus:border-primary-500 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
                                    id="category" name="category" type="text" placeholder="Kepala Desa" required
            ...
            <div class="relative max-h-full w-full max-w-md p-4">
                ...
                <div class="relative rounded-lg bg-white shadow dark:bg-gray-700">
                    ...
                    <form class="p-4 md:p-5" id="edit-form" method="POST"
                        action="{{ route('admin.staffcategory.update', ['staffcategory' => ':id']) }}">
                        @method('PUT')
                        @csrf
                        <div class="mb-4 grid grid-cols-2 gap-4">
                            <div class="col-span-2">
                                ...
                                <input
                                    class="focus:ring-primary-600 focus:border-primary-600 dark:focus:ring-primary-500 dark:focus:border-primary-500 block w-full rounded-lg border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 dark:border-gray-500 dark:bg-gray-600 dark:text-white dark:placeholder-gray-400"
                                    id="category_update" name="category" type="text" placeholder="Kepala Desa"
                                    required
                                    oninput="this.value = this.value.replace(/\b\w/g, c => c.toUpperCase())" />
                            </div>
                        </div>
                ...
                editButtons.forEach(button => {
                    button.addEventListener('click', function() {
                        ...
                        form.action = form.action.replace(':id',
                            id); // Ganti placeholder dengan ID yang sesuai

                        // Set nilai input nama kategori
                        const inputCategory = document.querySelector('#category_update');
